<template>
  <el-container>
    ...
    <el-container class="container">
      <el-aside width="380px" class="column">
        <StartNode
          draggable="true"
...
export default {
  ...
  setup() {
    ...
    function exportEditor() {
      ...
      var chat_flow_id = localStorage.getItem("chat_flow_id");
      ...
      var config = {
        ...
        data: {
          chatbot_id: chat_flow_id,
          drawflow: draw_flow.drawflow,
        },
      };
      console.log("add chat flow config", config);
      console.log("add chat id", chat_flow_id);

      axios(config)
        .then(function (response) {
          console.log("add chat flow response", response.data);
          localStorage.removeItem("chat_flow_id");
        })
    ...
    function get_chat_bot_urls() {
      ...
      console.log("add chat bot urls config", config);

      axios(config)
        .then(function (response) {
          console.log("add chat bot urls response", response.data);
          selectURLs = response.data.map((file) => {
            return { label: file.name, value: file.file_url };
          });
 ...
 value = ${option.value}>${option.label}</option>`;
  ...
  >
    ...
    onMounted(() => {
      ...
      get_current_chat_bot();

      function get_current_chat_bot() {
        var chat_flow_id = localStorage.getItem("chat_flow_id");
        var config = {
          method: "get",
        ...
        console.log("get current chatbot config", config);
        ...
        axios(config)
          .then(function (response) {
            ...
            if (!response.data) {
              ...
              console.log("get current chatbot response", response.data);
              currentChatBot = JSON.parse(response.data.ui_data);
              draw_flow.drawflow = currentChatBot;
