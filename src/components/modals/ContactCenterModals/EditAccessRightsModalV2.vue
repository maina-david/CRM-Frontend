      ...
      >
        ...
        >
          ...
          >
            ...
            <slot>
              <div v-if="accessRightsGroupedFormatted?.length > 0">
                <tree-view
                  :data="accessRightsGroupedFormatted"
                  @myEvent="handleEvent"
                />
              </div>
...
export default {
  updated() {
    console.log("i am executing from updated");
    this.getAccessRightsFormatted({ profile_id: this.currentAccessRightId });
  },
  computed: {
    ...mapGetters([
      "accessRoot",
  ...
  props,
  components: { TreeView, ActionButton },
  methods: {
    ...mapActions([
      "assignAccessRightsToRoleProfile",
      "getAccessRightsFormatted",
      "addCheckedInputs",
    ]),

    handleEvent(message) {
      console.log("I am emitting the message", message);
    },
