<template>
  <div class="overflow-x-auto sm:rounded-lg p-1 mt-5">
    <div class="pt-2">
      <h1 class="font-semibold">Sales Teams</h1>
      <hr
      ...
      <tbody>
        ...
        >
          ...
          >
            <div>
              <button
                @click="handleEditSalesTeam(data, 'Edit')"
            ...
            <div v-if="data.active == '1'">
              <button
                @click="handleToggleActivateSalesTeam(data, 'DEACTIVATE')"
            ...
            <div v-if="data.active == '0'">
              <button
                @click="handleToggleActivateSalesTeam(data, 'ACTIVATE')"
            ...
            <div>
              <button
                @click="showSalesTeamUsers = !showSalesTeamUsers"
                class="font-medium text-xs text-center m-2 px-2 py-1 text-gray-500 rounded-sm hover:bg-lime-500 hover:text-white"
              >
                View Users
              </button>
  ...
  <CreateSalesTeam :state="state" :show="showCreateSalesTeam" />
  <SalesTeamUsersModal
    :show="showSalesTeamUsers"
    :current-sales-team="currentSalesTeam"
  />
</template>
...
import SalesTeamUsersModal from "@/components/modals/CRMModals/SalesTeamUsersModal.vue";
...
export default {
  ...
  setup() {
    ...
    const showSalesTeamUsers = ref(false);
  ...
  methods: {
    handleEditSalesTeam(data, state) {
      console.log(data, state);
      this.showCreateSalesTeam = !this.showCreateSalesTeam;
    },
    handleToggleActivateSalesTeam(data, action) {
      if (confirm("Confirm " + action + " from Sales Team?") == true) {
        console.log("de/activate sales teams", data.id);
      } else {
        console.log("de/activate sales teams cancelled");
      }
    },
  },
  components: { CreateSalesTeam, SalesTeamUsersModal },
};
</script>
