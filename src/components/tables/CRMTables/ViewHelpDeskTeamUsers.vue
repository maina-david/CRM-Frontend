...
export default {
  ...
  components: {},
  methods: {
    ...mapActions(["removeUserFromHelpDeskTeam"]),
    handleRemoveUser(data) {
      if (
        confirm("Confirm remove " + data.name + " from Help Desk Team?") == true
      ) {
        console.log("remove user from help desk teams", data.id);
        this.removeUserFromHelpDeskTeam({
          user_id: data.id,
          help_desk_team_id: this.helpDeskTeamId,
        });
      } else {
        console.log("remove user from help desk teams cancelled");
      }
      this.$emit("updatedModal", true);
    },
  },
};
</script>
