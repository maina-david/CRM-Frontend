<template>
  <div class="overflow-x-auto sm:rounded-lg p-1 mt-5">
    <div class="pt-4 flex justify-between">
      <div>
        <h1 class="font-semibold">Breaks</h1>
      </div>
      <div>
        <ActionButton
          text="Create Break"
          @click="handleCreateBreak('Create', {})"
        />
      </div>
    </div>
    <hr
      ...
      <tbody>
        ...
        >
          ...
          <td
            ...
            class="py-2 px-3 font-medium text-xs text-gray-900 whitespace-nowrap dark:text-white"
          >
            {{ data.maximum_allowed_time }}
          </td>
          <td
            scope="row"
            class="py-2 px-3 font-medium text-xs text-gray-900 whitespace-nowrap dark:text-white"
          >
            {{ data.allowed_per_day }}
          </td>
          <td
            scope="row"
            class="py-2 px-3 font-medium text-xs text-gray-900 whitespace-nowrap dark:text-white"
          ...
          <td>
            <button
              v-if="data.status == 'ACTIVE'"
              @click="handleBreakState(data, 'DISABLE')"
            ...
            <button
              v-else
              @click="handleBreakState(data, 'ENABLE')"
              class="font-medium text-xs m-2 text-bg-gray-500 hover:text-red-600"
            >
              Enable Break
            </button>
            <button
              @click="handleCreateBreak('Edit', data)"
              class="font-medium text-xs m-2 text-bg-gray-500 hover:text-green-600"
            >
              Edit
            </button>
          </td>
...
import CreateBreakModal from "../../modals/ContactCenterModals/CreateBreakModal.vue";
...
export default {
  ...
  components: { ActionButton, CreateBreakModal },
  setup() {
  ...
  methods: {
    ...mapActions(["updateBreakStatus"]),
    handleCreateBreak(state, data) {
      if (state == "Create") {
        data.name = "";
        data.description = "";
        data.maximum_allowed_time = "";
      ...
      this.currentBreak = data;
      this.showBreakModal = !this.showBreakModal;
    },
    handleBreakState(data, state) {
      if (confirm("Confirm " + state + " " + data.name + "?") == true) {
        console.log("change break state", data.id, state);
        this.updateBreakStatus({
          id: data.id,
        });
      } else {
        console.log("change break state cancelled");
      }
    },
  },
};
