<template>
  <div class="overflow-x-auto sm:rounded-lg p-1 mt-5">
    <div class="pt-4">
      <h1 class="font-semibold">User Assigned DIDs</h1>
      <hr
    ...
    >
      ...
      <tbody class="justify-center items-center">
        <tr
          ...
          class="bg-white border-b dark:bg-gray-800 text-center p-2 m-2 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
        >
        ...
        >
          ...
          <td
            ...
            class="py-2 px-3 font-medium text-xs text-gray-900 whitespace-nowrap dark:text-white"
          >
            {{ data.did == null ? "Not assigned" : data.did }}
          </td>

          <td class="py-2 px-3">
            <span>
              <button
                @click="handleAssignDID(data)"
                class="font-medium bg-blue-400 rounded-md text-xs text-white py-1 px-2 mr-1"
              >
                Assign DIDs
              </button>
            </span>
    ...
    <assignDiDModal :show="showModal" :currentUser="this.currentUser" />
  </div>
</template>
...
import { ref } from "vue";
import assignDiDModal from "@/components/modals/ContactCenterModals/assignDIDModal.vue";
...
export default {
  ...
  methods: {
    ...mapActions(["removeDids"]),
    handleAssignDID(data) {
