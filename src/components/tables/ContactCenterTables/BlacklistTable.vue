<template>
  <div class="overflow-x-auto sm:rounded-lg p-1 mt-5">
    <div class="pt-4">
      <h1 class="font-semibold">Blacklisted Contacts</h1>
      <hr
...
export default {
  ...
  components: {},
  methods: {
    ...mapActions(["removeFromBlacklist"]),
    whitelist(data) {
      if (
        confirm("Confirm Remove " + data.phone_number + " from Blacklist?") ==
        true
      ) {
        console.log("remove from blacklist", data.phone_number);
        this.removeFromBlacklist({
          black_list_id: data.id,
        });
      } else {
        console.log("remove from blacklist cancelled");
      }
    },
  },
};
