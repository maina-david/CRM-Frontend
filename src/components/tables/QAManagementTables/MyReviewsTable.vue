      ...
      <tbody>
        ...
        >
          ...
          <td
            ...
            class="py-2 px-3 font-medium text-xs text-gray-900 whitespace-nowrap dark:text-white"
          >
            {{ formatdate(data.interaction_time) }}
          </td>
          <td
            v-if="data.status === 'UNDER-REVIEW'"
            scope="row"
          ...
          >
            ...
            <div v-if="data.channel === 'Voice'">{{ data.channel }}</div>
          </td>

          <td v-if="data.status === 'UNDER-REVIEW'">
            <div>
              <button
                @click="handleReviewInteraction(data)"
                v-if="
...
export default {
  methods: {
    ...mapActions(["storeCurrentTicket", "getInteractionData"]),
    formatdate(data) {
      var moment_date = moment(data).format("YYYY-MM-DD, h:mm:ss a");
      return moment(moment_date).startOf("minute").fromNow();
    },
    handleReviewInteraction(data) {
      console.log("i am geting here");
      localStorage.setItem("current_interaction", JSON.stringify(data.id));
      this.getInteractionData({ id: data.interaction_reference });
    },
    format_time(data) {
      var moment_date = moment(data).format("YYYY-MM-DD, h:mm:ss a");
      return moment(moment_date).startOf("minute").fromNow();
