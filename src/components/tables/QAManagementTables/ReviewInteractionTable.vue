<template>
  <div class="overflow-x-auto sm:rounded-lg p-1 mt-5">
    <div
      ...
      class="border-b flex justify-between mt-3"
    >
      ...
      >
      <span class="text-xs"
        >Queue: {{ interaction.calls[0].call_detail[0].queue }}</span
      >
    </div>
    <div v-else class="border-b flex justify-between gap-5 text-sm mt-3">
      <div>
        <span class="font-semibold"
          >Chat Time: {{ format_date(interaction.conversation.date) }}</span
        >
        <span>Handling Time: {{ interaction.conversation.handling_time }}</span>
      </div>
      <div>
        <span>Queue: {{ interaction.conversation.queue }}</span>
        <span>Channel: {{ interaction.conversation.channel }}</span>
      </div>
    </div>
    <table
      ...
      <tbody>
        ...
        >
          ...
          >
            <div v-if="data.type == 'toggle'">
              <label class="inline-flex relative items-center cursor-pointer">
                <input
                  ...
                  value=""
                  class="sr-only peer"
                  @change="format_data(data, currentToggle)"
                />
                <div
            ...
            <div v-if="data.type == 'range'">
              <input
                type="range"
                @input="format_data(data, value)"
                v-model="value"
                :step="1"
                :min="0"
                :max="data.range"
                class="w-full accent-lime-500 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer range-sm dark:bg-gray-700"
              />
          ...
          >
            <div class="">
              <input
                v-model="comments[data.id]"
                @change="format_data_comments(data, value)"
                type="text"
                id="large-input"
                placeholder="Enter comment here ..."
                class="block w-full p-2 text-gray-900 border border-gray-200 rounded-sm bg-slate-50 sm:text-md focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              />
            </div>
          </td>
        </tr>
        ...
        >
          <div class="mt-3">
            <td
              scope="row"
              class="py-2 px-3 font-semibold text-sm text-gray-900 whitespace-nowrap dark:text-white"
            >
              <div>
                <button
