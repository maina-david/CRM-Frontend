<template>
  <div class="overflow-x-auto sm:rounded-lg p-1 mt-5">
    <div class="flex justify-between border-b">
      <div class="">
        <h2 class="font-bold text-xl p-3">Attachment Files</h2>
      </div>
    </div>
    <table
    ...
    >
      ...
      <tbody>
        ...
        >
          ...
          >
            <ActionButton
              @click="handleDeleteChatBotFile(data)"
              text="Delete"
              class="bg-red-500"
            />
          </td>
...
export default {
  ...
  methods: {
    ...mapActions(["deleteChatBotFiles"]),
    handleDeleteChatBotFile(data) {
      if (
        confirm("Confirm DELETE " + data.name + " from attachment files!") ==
        true
      ) {
        console.log("confirmed delete chatbot file", data.id);
        this.deleteChatBotFiles({
          file_id: data.id,
        });
      } else {
        console.log("cancel delete chatbot file", data.id);
      }
    },
  },
};
