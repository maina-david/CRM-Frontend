...
export default {
  ...
  components: {},
  methods: {
    ...mapActions(["removeUserFormoChatQueue"]),
    handleRemoveUser(data) {
      if (
        confirm("Confirm remove " + data.name + " from Chat Queue?") == true
      ) {
        console.log("remove user from chat queue", data.id);
        this.removeUserFormoChatQueue({
          user_id: data.id,
          chat_queue_id: this.chatQueueId,
        });
      } else {
        console.log("remove user from chat queue cancelled");
      }
