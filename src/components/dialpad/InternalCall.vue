<template>
  <div class="grid grid-rows-1 grid-flow-col gap-1">
    <div class="row-span-1 border-b">
      <div
        class="relative rounded-sm text-center overflow-hidden p-2 text-gray-700"
      >
        <div class="callPad">
          <div class="">
            <div class="pulstate">
              <div></div>
              <div></div>
              <div></div>
            </div>

            <div class="">
              <!-- <div class="name m-3" @click="togglePause(audioUrl)">Matt Sich</div> -->

              <div
                v-if="callSource == 'Outgoing'"
                class="status text-sm"
                data-dots="..."
                @click="toggleAudio(audioUrl)"
              >
                Calling Agent {{ agentData.name }}({{ agentData.sip_id }})...
              </div>
              <div
                v-if="callSource == 'Incoming'"
                class="status text-sm"
                data-dots="..."
                @click="toggleAudio(audioUrl)"
              >
                Incoming Call From {{ currentCallDetails.agent_name }}({{
                  currentCallDetails.sip
                }}) ...
              </div>
            </div>
          </div>

          <div>
            <div
              class="buttons grid grid-cols-3 gap-3 m-1 my-4 place-items-center justify-center"
            >
              <audio ref="call_audio"></audio>
              <div
                v-if="call_state == 'ANSWERED'"
                @click="muteCall()"
                class="ca-b-single m-1"
                data-label="Mute"
              >
                <span
                  class="material-icons p-2 hover:border-4 hover:border-blue-300 hover:rounded-full hover:shadow-2xl hover:text-blue-400"
                  >mic_off</span
                >
              </div>
              <div
                v-if="call_state == 'MUTED'"
                @click="unmute()"
                class="ca-b-single m-1 col-start-2"
                data-label="Unmute"
              >
                <span
                  class="material-icons p-2 hover:border-4 hover:border-blue-300 hover:rounded-full hover:shadow-2xl hover:text-blue-400"
                  >phone_in_talk</span
                >
              </div>
              <div
                v-if="call_state == 'ONHOLD'"
                @click="unhold()"
                class="ca-b-single m-1 col-start-2"
                data-label="Unmute"
              >
                <span
                  class="material-icons p-2 hover:border-4 hover:border-blue-300 hover:rounded-full hover:shadow-2xl hover:text-blue-400"
                  >phone_in_talk</span
                >
              </div>
              <div
                v-if="call_state == 'ANSWERED'"
                @click="holdCall()"
                class="ca-b-single m-1"
                data-label="Face to Face"
              >
                <span
                  class="material-icons p-2 hover:border-4 hover:border-blue-300 hover:rounded-full hover:shadow-2xl hover:text-blue-400"
                  >phone_paused</span
                >
              </div>
              <div
                v-if="call_state == 'ANSWERED'"
                class="ca-b-single m-1"
                data-label="Chat"
              >
                <span
                  @click="showSendMessageModal = !showSendMessageModal"
                  class="material-icons p-2 hover:border-4 hover:border-blue-300 hover:rounded-full hover:shadow-2xl hover:text-blue-400"
                  >chat</span
                >
              </div>
            </div>
          </div>
        </div>

        <div
          class="grid-cols-2 flex justify-between gap-3 items-center text-center"
        >
          <div v-if="call_state == 'RINGING'">
            <ActionButton text="Answer" @click="pickCall()" />
          </div>
          <div>
            <ActionButton text="Hangup" @click="hangUpCall()" />
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="row-span-1"></div>

  <SendMessageModal :show="showSendMessageModal" />
</template>

<script>
import $ from "jquery";
import { useStore } from "vuex";
import { ref, computed } from "vue";
import { mapActions, mapGetters } from "vuex";
import ActionButton from "@/components/ActionButton.vue";
import SendMessageModal from "@/components/modals/ContactCenterModals/SendMessageModal.vue";

export default {
  watch: {
    answerSipCall(val) {
      console.log("i am getting here to answer sip call from watch", val);
      if (this.answerSipCall == true) {
        this.pickUpCall();
        this.answerSIPCallState(false);
      }
    },
  },
  computed: {
    ...mapGetters([
      "current_session",
      "masked_remote_identity",
      "currentCallDetails",
      "answerSipCall",
    ]),
  },
  created() {
    if (this.callSource === "Outgoing") {
      console.log(
        "this is the agent data from created",
        localStorage.getItem("transferData")
      );
      this.agentData = JSON.parse(localStorage.getItem("transferData"));
      localStorage.setItem("click_to_Call_status", JSON.stringify("true"));
      this.clickToSip({ user_id: this.agentData.user_id });
    }
  },

  data() {
    return {
      mediaElement: "",
      remoteStream: "",
      agentData: {},
      //call_state: "RINGING",
    };
  },
  props: {
    callSource: { type: String },
  },

  agentDataInternal: {
    type: Object,
  },

  methods: {
    ...mapActions(["clickToSip", "answerSIPCallState", "answered"]),
    pickUpCall() {
      this.answerSIPCallState(false);
      var self = this;
      self.answered();
      self.current_session.accept();
      self.current_session.on("accepted", (data) => {
        console.log(data);
        self.answered();
        console.log("refs", self.$refs.call_audio);
        self.mediaElement = self.$refs.call_audio;
        self.remoteStream = new MediaStream();
        console.log(
          self.current_session.sessionDescriptionHandler.peerConnection.getReceivers()
        );
        self.current_session.sessionDescriptionHandler.peerConnection
          .getReceivers()
          .forEach((receiver) => {
            if (receiver.track) {
              self.remoteStream.addTrack(receiver.track);
            }
          });
        console.log("this call has been answered");
        console.log(self.remoteStream);
        console.log("mediaElement");
        console.log(self.mediaElement);
        self.mediaElement.srcObject = self.remoteStream;
        self.mediaElement.play();
      });
    },
  },
  setup() {
    var sendcall = false;
    var timeoutTimer = true;
    var timeCounter = 0;
    var timeCounterCounting = true;
    const showSendMessageModal = ref(false);
    const store = useStore();
    var current_session = store.getters.current_session;
    var muteCall = function () {
      store.dispatch("muted");
    };
    var unmute = function () {
      store.dispatch("muted");
    };
    var holdCall = function () {
      store.dispatch("onHold");
    };
    var unhold = function () {
      store.dispatch("onHold");
    };
    // eslint-disable-next-line no-unused-vars
    var timeCounterLoop = function () {
      if (timeCounterCounting) {
        setTimeout(function () {
          // eslint-disable-next-line no-unused-vars
          var timeStringSeconds = "";
          var minutes = Math.floor(timeCounter / 60);
          var seconds = timeCounter % 60;
          if (minutes < 10) {
            minutes = "0" + minutes;
          }
          if (seconds < 10) {
            seconds = "0" + seconds;
          }
          $(".status").text(minutes + ":" + seconds);
          timeCounter += 1;
          timeCounterLoop();
        }, 2000);
      }
    };
    // eslint-disable-next-line no-unused-vars
    var setToInCall = function () {
      $(".callPad").toggleClass("in-call");
    };
    var dots = 0;
    // eslint-disable-next-line no-unused-vars
    var looper = function () {
      if (timeoutTimer) {
        setTimeout(function () {
          if (dots > 3) {
            dots = 0;
          }
          var dotsString = "";
          for (var i = 0; i < dots; i++) {
            dotsString += ".";
          }
          $(".status").attr("data-dots", dotsString);
          dots += 1;
          looper();
        }, 500);
      }
    };
    //Function to hangup
    var hangUpCall = () => {
      store.dispatch("endedClickToCall");
      addAnimationToButton(this);
      timeCounterCounting = false;
      timeCounter = 0;
      timeoutTimer = false;
      $(".pulstate").toggleClass("active-call");
      $(".status").text("Hanging Up Call...");
      console.log("current session on hangup", current_session);
    };
    //Function that adds animation to buttons on click
    var addAnimationToButton = function (thisButton) {
      //add animation
      $(thisButton).removeClass("clicked");
      var _this = thisButton;
      setTimeout(function () {
        $(_this).addClass("clicked");
      }, 1);
    };
    var checkNumber = function () {
      // eslint-disable-next-line no-unused-vars
      var numberToCheck = $(".phoneString input").val();
    };
    var pickCall = function () {
      store.dispatch("answerSIPCallState", false);
      console.log("i am answering the call");
      store.dispatch("answerSIPCallState", true);
    };
    return {
      hangUpCall,
      pickCall,
      muteCall,
      unmute,
      holdCall,
      sendcall,
      unhold,
      checkNumber,
      showSendMessageModal,
      internalCall: computed(() => store.getters.internalCall),
      call_state: computed(() => store.getters.call_state),
      // current_session: computed(() => store.getters.current_session),
    };
  },
  components: {
    SendMessageModal,
    ActionButton,
  },
};
</script>

<style lang="css" scoped></style>
