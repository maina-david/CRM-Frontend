    ...
    <div class="grid grid-cols-4 grid-flow-col gap-2 mt-5">
      <div class="col-span-1 w-full">
        <div class="">
          <div class="bg-white max-w-xs w-full block rounded-sm shadow-sm p-2">
            ...
            <div v-for="acc in accountPending.data" :key="acc.id">
              <h2 id="accordion-collapse-heading-1">
                <div class="justify-center mr-4">
                  <button
                    type="button"
                    @click="handlePendingAccountDetails(acc)"
                  ...
                  >
                    <span class="text-sm text-left p-2"
                      ><span>{{ Object.values(acc)[1] }}</span></span
                    >
                    <div>
                      <span
                        v-if="acc.approval_type == 'CREATE'"
                        class="text-xs p-1 text-green-700 bg-green-100 rounded-sm"
                        >{{ acc.approval_type }}</span
                      >
                      <span
                        v-if="acc.approval_type == 'UPDATE'"
                        class="text-xs p-1 text-orange-700 bg-orange-100 rounded-sm"
                        >{{ acc.approval_type }}</span
                      >
                      <span
                        v-if="acc.approval_type == 'DECLINED'"
      ...
      >
        <div class="col-span-3 w-full">
          <UserPendingApprovalAccountDetailsCard :accounts="currentAccount" />
        </div>
      </div>
...
import UserPendingApprovalAccountDetailsCard from "@/components/cards/CRMCards/UserPendingApprovalAccountDetailsCard.vue";
import DeclineWithMessageModal from "@/components/modals/CRMModals/DeclineWithMessageModal.vue";
import Dropdown from "@/components/Dropdown.vue";

export default {
  name: "PendingApprovalAccounts",
  components: {
    UserPendingApprovalAccountDetailsCard,
  ...
  created() {
    this.getAccountsPending();
  },
  methods: {
    ...mapActions(["getAccountsPending"]),
    handlePendingAccountDetails(data) {
      this.currentAccount = data;
    },
  },
  computed: {
    ...mapGetters(["accountPending"]),
  },
};
</script>
