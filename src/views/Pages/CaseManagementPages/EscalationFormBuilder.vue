    ...
    <div class="m-5">
      <div class="border-b mt-5">
        <div class="flex justify-between m-4">
          <div>
            <h2 class="font-bold">{{ current_form_data.name }}</h2>
            <h2 class="text-xs my-2">
              {{ current_form_data.description }}
            </h2>
          </div>
...
export default {
  created() {
    console.log(
      "new form",
      JSON.parse(localStorage.getItem("current_escalation_form"))
    );
    this.current_form_data = JSON.parse(
      localStorage.getItem("current_escalation_form")
    );
  },
  data() {
    return {
  ...
  methods: {
    ...mapActions(["addCENTRALFormItems"]),
    handleCreateFormItems() {
      ...
      if (confirm("Confirm Create Account Form") == true) {
        console.log("create account form", this.formInfo);
        this.addCENTRALFormItems({
          form_id: this.current_form_data.id,
          form_items: this.formInfo["form_items"],
        });
      } else {
