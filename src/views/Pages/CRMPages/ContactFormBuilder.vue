        ...
        <div class="flex justify-between m-4">
          ...
          <div class="flex gap-3">
            ...
            <div class="export_json">
              <ActionButton @click="handleCreateFormItems" text="Submit Form" />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...mapActions(["createContactFormItems"]),
    handleCreateFormItems() {
      this.formInfo = JSON.parse(localStorage.getItem("Contact Form Info"));
      this.formInfo = this.formInfo[0];
      this.formInfo.contact_form_id = JSON.parse(
        localStorage.getItem("currentContactFormId")
      );
      this.formItems = JSON.parse(localStorage.getItem("account_form_builder"));

      if (confirm("Confirm Create Contact Form") == true) {
        console.log("create contact form");
        this.createContactFormItems(this.formInfo);
      } else {
        console.log("create contact form cancelled");
      }
    },
  },
  watch: {
  ...
  setup() {
    ...
    $(document).ready(function () {
      if (localStorage.getItem("Contact Form Info") != null) {
        var prevJson = localStorage.getItem("Contact Form Info");
    ...
    $(document).on("click", ".export_html", function () {
      ...
      localStorage.setItem("Contact Form Info", JSON.stringify(json));
    });
