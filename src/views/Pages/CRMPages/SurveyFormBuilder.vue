    ...
    <div class="m-5">
      <div class="border-b mt-5">
        <div class="flex justify-between m-4">
          <div>
            <h2 class="font-bold">{{ currentSurveyForm.name }}</h2>
            <h2 class="text-xs my-2">
              {{ currentSurveyForm.description }}
            </h2>
          </div>
...
export default {
  computed: {
    ...mapGetters(["currentSurveyForm"]),
  },
  methods: {
    ...mapActions(["createCENTRALFormItems", "addCENTRALFormItems"]),
    handleCreateFormItems() {
      ...
      this.formInfo.account_form_id = this.currentSurveyForm.id;

      this.addCENTRALFormItems({
        form_id: this.currentSurveyForm.id,
  ...
  setup() {
    ...
    $(document).on("click", ".export_json", function () {
      var json = "";
      json = getJson();
      console.log("i am submitting json", json);
      localStorage.setItem("Form Info", JSON.stringify(json));
    });
